...
uint8_t findBestInQuantizator(const uint8_t value, const std::vector<std::pair<uint8_t, uint8_t>> quantizator)
{
    int minDistIndex = -1;
    uint8_t minDist = UINT8_MAX;
    ...
    for (int j = 0; j < quantizator.size(); j++)
    {
        ...
        uint8_t tmp = getAreaCenter(quantizator[j].first, quantizator[j].second);
        if (value >= tmp)
            dist = value - tmp;
        else 
            dist = tmp - value;
    ...
    if (minDistIndex > 0)
    {
        return getAreaCenter(quantizator[minDistIndex].first, quantizator[minDistIndex].second);
    }

    //never claim
    return getAreaCenter(quantizator[0].first, quantizator[0].second);
}
...
void encodeImage(const char* outputFileName, const char* inputFileName, uint8_t quantizationBits)
{
    ...
    // calculate {Y} and {Z}
    //znajdz kwantyzator dla y
    std::vector<std::pair<uint8_t, uint8_t>> yQuantizator[COLOR_COMPONENTS];
    std::vector<uint8_t> zQuantizator[COLOR_COMPONENTS];
    ...
    for (int j = 0; j < COLOR_COMPONENTS; j++)
    {
        ...
        yQuantizator[j] = rownomiernyPair(quantizationBits);
        std::vector<std::seed_seq::result_type> yFrequency(yQuantizator[j].size(), 0); 
        //frequency starts at one so that an empty merged area is bigger than an empty smaller one
        ...
        for (int i = 1; i < y[j].size(); i++)
        {
            ...
            for (int k = tmp; (uint8_t)(currentY + tmp) < currentY && y[j][i] > y[j][i - 1]; k--)
            {
                // std::cout << " inside " << std::endl;  

                tmp = findBestInQuantizator(k, yQuantizator[j]);
            }
            ...
            //     tmp = findBestInDynamicQuantizator(-k, &yQuantizator[j], &yFrequency, yCharFrequency);
            // }
            newY[i] = tmp;
