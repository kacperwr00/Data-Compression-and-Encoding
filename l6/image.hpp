...
void writeImageToFile(const char* outputFile, image* outputImg, const char* inputFile)
{
    //copy the header from the inputFile to the output
    //complies with assignment reqiurments even though we would need a color map to compress the file
    FILE* output = fopen(outputFile, "w");

    FILE* input = fopen(inputFile, "r");
    const int headerSize = 18;
    int pixelCount = outputImg->height * outputImg->width;

    char buffer[headerSize];
    if (!fread(buffer, sizeof(*buffer), headerSize, input) == sizeof(*buffer))
    {
        std::cout << "Failed reading header from input" << std::endl;
        return;
    }
    fwrite(buffer, sizeof(*buffer), headerSize, output);

    //write bitmap
    for (int i = 0; i < pixelCount; i++)
    {
        fwrite(&outputImg->bmp[i * COLOR_COMPONENTS], sizeof(*(outputImg->bmp)), COLOR_COMPONENTS, output);
    } 

    //footer probably unnecessary
