...
void writeImageToFile(FILE* outputFile, image* outputImg, const char* inputFile)
{
    ...
    fread(buffer, sizeof(*buffer), headerSize, input);
    fwrite(buffer, sizeof(*buffer), headerSize, outputFile);

    //write bitmap
    for (int i = 0; i < pixelCount; i++)
    {
        fwrite(&outputImg->bmp[i * COLOR_COMPONENTS], sizeof(*(outputImg->bmp)), COLOR_COMPONENTS, outputFile);
    } 
